...
const overrideApiBase = (process.env.NEXT_PUBLIC_OVERRIDE_API_URL || "").trim().replace(/\/$/, "");
            ...
            const chainWithSprites = await Promise.all(
              chain.slice(0, 6).map(async (sname) => {
                try {
                  ...
                  let overrideArt = null;
                  try {
                    const ov = await fetchOverrideEntry(p2.id ?? sname, controller.signal);
                    // For evolution boxes, prefer override artwork only; do not fall back to override sprite.
                    overrideArt = ov?.artNormal || null;
                  } catch (err) {
                    void err;
                  }
                  const sprite =
